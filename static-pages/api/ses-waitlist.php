...
// AWS SES Configuration (same as your WordPress plugin)
$awsConfig = [
    'region' => 'us-east-1', // Update this to match your WordPress plugin settings
    'version' => 'latest',
    'credentials' => [
        'key' => '', // Your AWS Access Key ID from WordPress plugin
        'secret' => '', // Your AWS Secret Access Key from WordPress plugin
    ]
];
...
// Simple email function using AWS SES via API
function sendEmailViaSES($to, $subject, $htmlBody, $textBody = null) {
    global $awsConfig;
    
    // If no AWS SDK available, fall back to simple mail
    if (!class_exists('Aws\Ses\SesClient')) {
        return sendSimpleEmail($to, $subject, $textBody ?: strip_tags($htmlBody));
    }
    
    try {
        require_once '/mnt/c/xampp/htdocs/wordpress/wp-content/plugins/wp-mail-ses/vendor/autoload.php';
        
        $sesClient = new Aws\Ses\SesClient($awsConfig);
...
// Send emails
$userEmailSent = sendEmailViaSES($email, $userSubject, $userHtmlMessage, $userTextMessage);
$adminEmailSent = sendEmailViaSES($adminEmail, $adminSubject, null, $adminMessage);
